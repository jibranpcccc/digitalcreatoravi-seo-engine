#!/usr/bin/env python3
"""Register Site 3 (OpenAgentStack) and Site 4 (IndieStackAudit) in Google Search Console."""
...
def verify_and_add_site(ws, target_url, short_name):
    ...
    if not (isinstance(url_input, str) and url_input.startswith("{")):
        print("Could not find URL prefix input, checking fallback...")
        url_input = ev_raw(ws, ctx, """(() => {
            const inps = Array.from(document.querySelectorAll('input[type=text], input:not([type])')).filter(i => i.offsetParent);
            if (!inps.length) return null;
    ...
    shot(ws, ctx, f"40_verification_modal_{short_name}.png")
    ...
    if isinstance(verif_btn, str) and verif_btn.startswith("{"):
        ...
        time.sleep(6)
        shot(ws, ctx, f"41_after_verify_{short_name}.png")
    ...
    if isinstance(go_btn, str) and go_btn.startswith("{"):
        ...
        time.sleep(3)
        shot(ws, ctx, f"42_property_dashboard_{short_name}.png")
    ...
    time.sleep(10)
    ctx = F.get_ctx(ws)
    shot(ws, ctx, f"43_sitemap_page_{short_name}.png")
    ...
    if isinstance(sinp, str) and sinp.startswith("{"):
        ...
        time.sleep(2)
        shot(ws, ctx, f"44_sitemap_typed_{short_name}.png")
        ...
        if isinstance(s_sub, str) and s_sub.startswith("{"):
            ...
            time.sleep(6)
            shot(ws, ctx, f"45_sitemap_submitted_{short_name}.png")
            ...
            time.sleep(3)
            shot(ws, ctx, f"46_sitemap_final_{short_name}.png")
...
def main():
    sites = [
        {"url": "https://openagentstack.pages.dev/", "name": "OpenAgentStack"},
        {"url": "https://indiestackaudit.pages.dev/", "name": "IndieStackAudit"}
    ]
