...
<body class="min-h-screen text-slate-200 antialiased p-3 sm:p-6 lg:p-8 selection:bg-cyan-500 selection:text-slate-950 font-sans">
  ...
  <header class="max-w-7xl mx-auto mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4 glass p-5 rounded-2xl shadow-2xl">
    <div class="flex items-center gap-4">
      ...
      <div>
        <div class="flex flex-wrap items-center gap-2">
          ...
          <span class="hidden sm:inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 text-[11px] font-mono font-bold">
            🛡️ 100% REAL TELEMETRY • ZERO SIMULATION
          </span>
        </div>
        <p class="text-xs text-slate-400 mt-0.5">
          Unified Multi-Host Webmaster Hub & Real Visitor Analytics • 8 Isolated Google Search Console Accounts
        </p>
      </div>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2.5">
      ...
      <a href="https://webhookwatch.vercel.app/api/stats" target="_blank" class="px-3.5 py-2 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/30 text-purple-300 text-xs font-mono transition flex items-center gap-1.5 shadow-sm">
        <span>🌐 Edge Relay Feed</span> ↗
      </a>
    </div>
  </header>
  ...
  <div class="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
    ...
    <div class="glass-card p-4 rounded-xl border-l-4 border-l-amber-500">
      <div class="flex items-center justify-between text-[11px] text-slate-400 font-mono mb-1">
        <span>REAL VISITOR HITS</span>
        <span class="text-amber-400 font-bold">● Live Relay</span>
      </div>
      <div class="text-2xl font-black text-amber-300 font-mono" id="stat-total-hits">0</div>
      <div class="text-[10px] text-slate-500 mt-0.5 truncate" id="stat-organic-pct">100% Real Edge Hits</div>
    </div>
  ...
  <div class="max-w-7xl mx-auto mb-6">
    <div class="flex items-center gap-1 border-b border-slate-800/80 overflow-x-auto pb-0.5">
      ...
      <button onclick="switchTab('traffic')" id="tab-btn-traffic" class="tab-inactive px-4 py-3 text-xs sm:text-sm rounded-t-xl transition flex items-center gap-2 whitespace-nowrap">
        <span>📊</span> Real Traffic Intelligence
      </button>
  ...
  <main class="max-w-7xl mx-auto space-y-6">
    ...
    <div id="tab-pane-fleet" class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-slate-800/80">
          <div>
            <h2 class="text-base sm:text-lg font-black text-white flex items-center gap-2 tracking-tight">
              <span>🛡️</span> All 20 Production Sites — Real Traffic & Webmaster Account Matrix
            </h2>
            <p class="text-xs text-slate-400 mt-1">
              Verified real visitor hits from production edge relay, dedicated GSC accounts, and verified sitemaps.
            </p>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <select id="fleet-sort-filter" onchange="filterFleetTable()" class="bg-slate-900 border border-slate-700/80 rounded-xl px-3 py-1.5 text-xs text-slate-300 focus:outline-none focus:border-cyan-500">
              ...
              <option value="hits-desc">Sort: Real Traffic (High to Low)</option>
          ...
          <table class="w-full text-left text-xs border border-slate-800/80 rounded-xl overflow-hidden">
            <thead class="bg-slate-900/90 text-white font-semibold font-mono uppercase tracking-wider text-[11px]">
              <tr>
                ...
                <th class="p-3.5 border-b border-slate-800 text-right">Real Traffic Hits</th>
      ...
      <div class="glass p-6 rounded-2xl border-l-4 border-l-cyan-500">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 text-xs font-mono" id="isolation-grid"></div>
      </div>
    </div>
    ...
    <div id="tab-pane-traffic" class="space-y-6 hidden">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="glass p-6 rounded-2xl">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <span>📊</span> Real Traffic Acquisition
            </h2>
            <span class="text-xs text-cyan-400 font-mono font-bold" id="traffic-source-total">0 Hits</span>
          </div>
        ...
        <div class="glass p-6 rounded-2xl">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <span>🌐</span> Real Hits by Production Site
            </h2>
            <span class="text-xs text-slate-400 font-mono">100% Verified Only</span>
          </div>
          <div class="h-60 relative flex items-center justify-center">
        ...
        <div class="glass p-6 rounded-2xl flex flex-col justify-between">
          <div>
            <div class="flex items-center justify-between mb-3">
              <h2 class="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                <span>⚡</span> Real-Time Visitor Stream
              </h2>
              <span class="inline-flex items-center gap-1 text-[11px] text-emerald-400 font-mono">
                <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 pulse-dot"></span> Live Relay
              </span>
            </div>
            <div id="live-hit-feed" class="space-y-2 max-h-60 overflow-y-auto pr-1 text-xs font-mono">
              <div class="text-slate-500 text-center py-10">Awaiting live browser visitor telemetry...</div>
            </div>
          </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="glass-card p-4 rounded-xl">
          ...
          <div class="text-2xl font-black text-emerald-400 font-mono mt-1" id="stat-source-organic">0</div>
        ...
        <div class="glass-card p-4 rounded-xl">
          ...
          <div class="text-2xl font-black text-purple-400 font-mono mt-1" id="stat-source-social">0</div>
        ...
        <div class="glass-card p-4 rounded-xl">
          ...
          <div class="text-2xl font-black text-cyan-400 font-mono mt-1" id="stat-source-direct">0</div>
        ...
        <div class="glass-card p-4 rounded-xl">
          ...
          <div class="text-2xl font-black text-amber-400 font-mono mt-1" id="stat-source-referral">0</div>
  ...
  <script>
    ...
    function filterFleetTable() {
      ...
      const sortFilter = document.getElementById('fleet-sort-filter')?.value || 'id-asc';
    ...
    function renderFleetTable(sites) {
      ...
      sites.forEach(s => {
        ...
        tr.innerHTML = `
          <td class="p-3.5">
            <div class="flex items-center gap-2">
              <span class="w-2 h-2 rounded-full ${hits > 0 ? 'bg-emerald-400 pulse-dot' : 'bg-slate-500'}"></span>
          ...
          <td class="p-3.5 text-right font-mono">
            <div class="text-sm font-black ${hits > 0 ? 'text-emerald-400' : 'text-slate-400'}">
              ${hits.toLocaleString()} hits
            </div>
            <div class="text-[10px] text-slate-500 mt-0.5">
              ${hits > 0 ? 'Verified Real' : 'Awaiting Visitors'}
            </div>
          </td>
    ...
    async function loadStatsData() {
      try {
        ...
        if (data.ok) {
          ...
          const orgPct = totalHits > 0 ? ((org / totalHits) * 100).toFixed(1) : '0';
          document.getElementById('stat-organic-pct').innerText = totalHits > 0 ? `${orgPct}% Organic Search` : '100% Real Edge Relay';
    ...
    function renderCharts(bySource, bySite) {
      ...
      const siteLabels = entries.length > 0 ? entries.map(e => e[0]) : ['No Visitors Yet'];
      const siteValues = entries.length > 0 ? entries.map(e => e[1]) : [0];
    ...
    function renderHitFeed(recentHits) {
      ...
      if (!recentHits || recentHits.length === 0) {
        feed.innerHTML = '<div class="text-slate-500 text-center py-10">Awaiting live browser visitor telemetry...</div>';
        return;
      }
